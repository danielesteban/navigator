...
namespace ImGui
{
    ...
    enum MarkdownColors {
      ImGuiMarkdownCol_Text,
      ImGuiMarkdownCol_Emphasis_1,
      ImGuiMarkdownCol_Emphasis_2,
      ImGuiMarkdownCol_Heading,
      ImGuiMarkdownCol_Link,
      ImGuiMarkdownCol_LinkHovered,
      ImGuiMarkdownCol_COUNT
    };
    ...
    struct MarkdownConfig
    {
        ...
        ImVec4 Colors[ImGuiMarkdownCol_COUNT] = {
          ImVec4(0.8, 0.8, 0.8, 1.0),
          ImVec4(0.6, 0.6, 0.6, 1.0),
          ImVec4(0.45, 0.45, 0.45, 1.0),
    ...
    inline void Markdown( const char* markdown_, size_t markdownLength_, const MarkdownConfig& mdConfig_ )
    {
        ImGui::PushStyleColor( ImGuiCol_Text, mdConfig_.Colors[ ImGuiMarkdownCol_Text ] );
    ...
    inline void defaultMarkdownFormatCallback( const MarkdownFormatInfo& markdownFormatInfo_, bool start_ )
    {
        ...
        case MarkdownFormatType::EMPHASIS:
        {
            ...
            if( markdownFormatInfo_.level == 1 || markdownFormatInfo_.level == 2 )
            {
                // normal emphasis
                if( start_ )
                {
                    ImGui::PushStyleColor( ImGuiCol_Text, markdownFormatInfo_.config->Colors[ markdownFormatInfo_.level == 1 ? ImGuiMarkdownCol_Emphasis_1 : ImGuiMarkdownCol_Emphasis_2 ]  );
                }
                else
                {
                    ImGui::PopStyleColor();
        ...
        case MarkdownFormatType::HEADING:
        {
            ...
            if( start_ )
            {
                ...
                // ImGui::NewLine();
                ImGui::PushStyleColor( ImGuiCol_Text, markdownFormatInfo_.config->Colors[ ImGuiMarkdownCol_Heading ] );
            }
            else
            {
                if( fmt.separator )
                ...
                else
                {
                    ...
                    if (markdownFormatInfo_.level == 1) {
                      ImGui::SetCursorPos(ImVec2(p.x, p.y + 8));
                    }
                }
                if( fmt.font )
                {
        ...
        case MarkdownFormatType::LINK:
            if( start_ )
            {
                ImGui::PushStyleColor( ImGuiCol_Text, markdownFormatInfo_.config->Colors[ ImGuiMarkdownCol_LinkHovered ] );
            }
            else
            {
                ImGui::PopStyleColor();
                ...
                {
                    ImGui::UnderLine( markdownFormatInfo_.config->Colors[ ImGuiMarkdownCol_LinkHovered ] );
                }
                else
                {
                    ImGui::UnderLine( markdownFormatInfo_.config->Colors[ ImGuiMarkdownCol_Link ] );
                }
            }
            break;
        }
